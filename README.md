...
- 自動接続先以外のチャンネルでは、話者が変わった時だけ発言者名を読み上げ(Discordの連続投稿表示に倣い、同じ人の連続投稿では名前を省略)
- ユーザー・ロール・チャンネルメンションやカスタム絵文字を自然な名称に変換して読み上げ
- 本文中のリンク先(YouTube・Xの投稿含む)のタイトルを取得して「リンク省略(タイトル)」のように読み上げ。Discordが生成するリンク埋め込み情報を優先的に利用し、取得できない場合のみ直接アクセスを試行(プライベートIP等へのアクセスは自動的にブロック)、それでも取得できない場合はドメイン名を使った「リンク省略。(ドメイン名)」にフォールバック
